...
export function WhatsappFloatingButton() {
  ...
  return (
    ...
    >
      {expanded ? (
        <div className="flex max-w-[17rem] items-start gap-3 rounded-[16px] border border-line bg-white px-4 py-3 shadow-[0_14px_36px_-28px_rgba(23,32,29,0.55)]">
          ...
          <button
            ...
            className="-mr-1 -mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-[10px] text-ink-subtle transition-colors duration-200 hover:bg-mist hover:text-ink"
          >
      ...
      <a
        ...
        className="group inline-flex min-h-[52px] items-center gap-3 rounded-[16px] border border-forest bg-forest pl-4 pr-5 text-white shadow-[0_14px_36px_-24px_rgba(29,51,43,0.85)] transition-colors duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-forest-deep"
      >
        <WhatsappGlyph className="h-[22px] w-[22px] shrink-0" />
        <span className="flex flex-col leading-tight">
          <span className="text-small font-medium">WhatsApp us</span>
          {display ? (
            <span className="text-label text-white/70">{display}</span>
          ) : null}
        </span>
      </a>
    </div>
